...
function init() {
    ...
    clcontext = {
					...
					npcap = npcap + parseFloat(feature.cluster[i].data.neutre);
                                ...
                                return 20 + Math.round(symbol.getSize(npcap) * Math.pow(2,map.getZoom()-1));
                             },
				   ...
				   negif += neg;
				    ...
				    // alert(jsontext);
				     ...
				     if(parseInt(neg)==0 && parseInt(neutre)==0){
                                    ...
                                    charturl = 'http://chart.apis.google.com/chart?cht=it&chd=t:100&chs=150x150&chco=FDF1B8,FFFFFF00,FFFFFF00&chf=bg,s,FFFFFF00';
				     }
				     else{
				     
                                    charturl = 'http://chart.apis.google.com/chart?cht=p&chd=t:'+values+'&chs=150x150&chco=096A09,FF0000,FDF1B8&chf=bg,s,ffffff00';
				      //alert(charturl);
				     }
...
/*      var suchobjekte = [];
  ...
  */
	...
	var filterss = new OpenLayers.Filter.Logical({
	    ...
	    evaluate : clcontext,
	    featureType: "filteropinionnew",
            filters: getOtherAttributesFilter(y)
        });
...
var rule = new OpenLayers.Rule({
 ...
 wfsLayer.events.register('featuresadded', wfsLayer, function() {
   initializeChart();
    ...
    map.addLayers([gsat, osmLayer, gmap ,wfsLayer]);
