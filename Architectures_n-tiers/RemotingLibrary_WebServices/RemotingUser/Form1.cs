    ...
    public partial class Form1 : Form
    {
        ...
        private void buttonSearchIsbn_Click(object sender, EventArgs e)
        {
            if (remoteOperation != null)
            {
                ...
                if (isbn.Length != 0)
                {
                    if (!remoteOperation.SearchISBN(isbn).Title.Equals("null"))
                    {
                        MessageBox.Show("Le livre est dans le catalogue", "Détails du livre", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        textBoxIsbn.Clear();
                    }
                    else
                    {
        ...
        private void buttonSearchAuthor_Click(object sender, EventArgs e)
        {
            if (remoteOperation != null)
            {
                ...
                if (auteur.Length != 0)
                {
                    if (!remoteOperation.SearchAuthor(auteur).Title.Equals("null"))
                    {
                        MessageBox.Show("Le livre est dans le catalogue", "Détails du livre", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        textBoxAuthor.Clear();
                    }
